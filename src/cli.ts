...
import { login, logout, waitForLocks, whoami, publish, authenticated } from './lib'
    ...
    .command(
        ...
        async () => {
            try {
                ...
                const correctVersion = await validateVersion(version) || version == ""
                if (!correctVersion) {
                ...
                await publish()
                console.log(chalk.greenBright`Published`, chalk.bold.blueBright`${pkg['name']}@${pkg['version']}`)
                console.log(chalk.italic.gray`https://aspkg.dev/package/${pkg['name']}`)
                await gracefulShutdown()
            } catch (e) {
